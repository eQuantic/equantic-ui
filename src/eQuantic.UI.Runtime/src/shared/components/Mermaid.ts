import { Box, BoxStyle, BuildContext, CodeBlock, ColorToken, CornerRadii, EdgeInsets, IconGlyph, MermaidArrowhead, MermaidLayout, MermaidParser, MermaidPlacedNode, Positioned, ScrollView, SizeValue, Stack, StatelessComponent, Text, Vector } from "../runtime-exports";
...
export class Mermaid extends StatelessComponent {
    ...
    build(context: BuildContext) {
        ...
        let graph = MermaidParser.parse(this.source);
        if (graph == null) return new CodeBlock(this.source, 'mermaid', { showLineNumbers: false });
        let scene = MermaidLayout.solve(graph);
        let canvas = new Stack('topStart', { width: scene.width, height: scene.height });
        let ink = theme.borderStrong;
        for (const curve of scene.curves) canvas.add(new Positioned(new Vector(new IconGlyph('edge', curve.path, 'stroke', curve.viewBox, Mermaid.edgeWidth), curve.w, ink, null, curve.h), curve.y, null, null, curve.x));
        for (const segment of scene.segments) canvas.add(new Positioned(new Box(new BoxStyle({ width: segment.w, height: segment.h, background: ink })), segment.y, null, null, segment.x));
    ...
    static headGlyph(direction: number) {
        if (direction === 1) return new IconGlyph('mermaidHeadRight', 'M0 0 V100 L100 50 Z', 'fill', '0 0 100 100');
        if (direction === 2) return new IconGlyph('mermaidHeadUp', 'M50 0 L100 100 H0 Z', 'fill', '0 0 100 100');
        if (direction === 3) return new IconGlyph('mermaidHeadLeft', 'M100 0 V100 L0 50 Z', 'fill', '0 0 100 100');
        return new IconGlyph('mermaidHeadDown', 'M0 0 H100 L50 100 Z', 'fill', '0 0 100 100');
    }
