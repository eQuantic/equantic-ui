...
export class CodeEditor extends StatefulComponent {
    ...
    build(context: BuildContext) {
        ...
        let surface: VisualNode = new CodeSurface(block, editor, { autofocus: this.autofocus, label: this.caption ?? 'Code editor', caretColor: CodeBlock.inkFor(this.inverse, context.theme), onChanged: () => this.setState(() => {
        ...
        surface = new Box(new BoxStyle({ width: SizeValue.fill, background: CodeBlock.surfaceFor(this.inverse, context.theme), cornerRadius: new CornerRadii(context.theme.shape('medium')), clip: true }), viewport);
        surface = new Shortcut(surface, new KeyChord('f', 4), () => this.setState(() => this._findOpen = true));
        if (!this._findOpen) return surface;
        let layers = new Stack('topStart', { width: SizeValue.fill });
        layers.add(surface);
        layers.add(new Positioned(this.findBar(context, editor), 8, 8));
        return layers;
    }
    ...
    marks(editor: CodeEditorController) {
        ...
        if (((needle != null && needle.length > 0) && (search = needle, true))) {
            ...
            for (const match of editor.findAll(search, this.searchMatchCase)) {
    ...
    findBar(context: any, editor: CodeEditorController) {
        const step = (forward: boolean) => {
            ...
            let match = editor.findNext(this._findText, this.searchMatchCase, !forward);
            let found: any; 
            if ((found = match) != null) this.setState(() => editor.selection = found);
        };
        let theme = context.theme;
        let matches = this._findText.length > 0 ? editor.findAll(this._findText, this.searchMatchCase).length : 0;
        let index = 0;
        if (matches > 0) {
            let current = editor.selection;
            let all = editor.findAll(this._findText, this.searchMatchCase);
            for (let i = 0; i < all.length; i++) if ($eq.equals(all[i].start, current.start)) {
                index = i + 1;
                break;
            }
        }
        ...
        row.add(new IconButton(new Icon(IconGlyph.fromIcons('close')), 'Close find', 'standard', 'medium', null, { size: 'small', onPressed: () => this.setState(() => {
