import { $eq, Box, BoxStyle, BuildContext, CornerRadii, Icon, Image, Positioned, Sizing, Stack, StatelessComponent, Text, TypeStyle, VisualNode } from "../runtime-exports";
...
export class Avatar extends StatelessComponent {
    ...
    build(context: BuildContext) {
        let theme = context.theme;let side = Sizing.avatar(this.size);let labelSize = (() => { const _s = this.size; if (_s === 'small') return 10; if (_s === 'medium') return 13; if (_s === 'large') return 16; return 22; })();let source: any; if (((this.imageSource != null) && (source = this.imageSource, true))) {let photo = new Image(source, side, side, 'cover', this.name ?? this.initials, { cornerRadius: new CornerRadii(theme.shape('full')) });return this.status === 'none' ? photo : this.withStatusDot(photo, side, theme);}let seed = this.name ?? this.initials;let tint = theme.colors(Avatar.tintPalette[seed.length % Avatar.tintPalette.length]);let clipped = this.initials.length > 2 ? this.initials.substring(0, 0 + 2) : this.initials;let hasInitials = clipped.length > 0;let glyphSize = (() => { const _s = this.size; if (_s === 'small') return 16; if (_s === 'medium') return 20; if (_s === 'large') return 24; return 32; })();let face = hasInitials ? new Text(clipped, 'caption', tint.onSubtle, 1, 'start', false, false, null, { styleOverride: new TypeStyle(labelSize, labelSize, 'semiBold', 0, 1.3) }) : new Icon('person', glyphSize, theme.textMuted);let content = face.centered();let circle = new Box(new BoxStyle({ width: side, height: side, background: hasInitials ? tint.subtle : theme.surfaceSubtle, cornerRadius: new CornerRadii(theme.shape('full')) }), content);return this.status === 'none' ? circle : this.withStatusDot(circle, side, theme);
    }
