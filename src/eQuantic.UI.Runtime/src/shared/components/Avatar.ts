...
export class Avatar extends StatelessComponent {
    ...
    build(context: BuildContext) {
        ...
        let clipped = this.initials.length > 2 ? this.initials.substring(0, 0 + 2) : this.initials;
        ...
        let face = hasInitials ? new Text(clipped, 'caption', tint.onSubtle, 1, 'start', false, false, null, { styleOverride: new TypeStyle(labelSize, labelSize, 'semiBold', 0, 1.3) }) : new Icon('person', glyphSize, theme.textMuted);
    ...
    withStatusDot(face: VisualNode, side: number, theme: any) {
        let dotSide = $eq.math.round(side / 3.3);
