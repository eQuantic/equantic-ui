...
export class Pagination extends StatelessComponent {
    ...
    build(context: BuildContext) {
        let theme = context.theme;let current = Math.min(Math.max(this.currentPage, 1), Math.max(1, this.pageCount));let numbers = new Row(3, 'start', 'center', false, null, null, { cross: 'center' });let previous = 0;for (const page of Pagination.pages(this.pageCount, current)) {if (previous > 0 && page > previous + 1) numbers.add(Pagination.ellipsis(theme));numbers.add(this.number(theme, page, page === current));previous = page;}let row = new Row(8, 'start', 'center', false, null, null, { cross: 'center' });row.add(Pagination.step('Previous', current > 1, () => this.onChanged?.(current - 1)));row.add(numbers);row.add(Pagination.step('Next', current < this.pageCount, () => this.onChanged?.(current + 1)));return row;
    }
    ...
    static ellipsis(theme: any) {
        let centered = new Row(0, 'start', 'center', false, null, null, { width: SizeValue.fill, height: SizeValue.fill, main: 'center', cross: 'center' });centered.add(new Text('…', 'caption', theme.textMuted, 1));return new Box(new BoxStyle({ width: Pagination.cell, height: Pagination.cell }), centered);
    }
