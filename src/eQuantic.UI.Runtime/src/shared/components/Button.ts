import { Box, BoxStyle, BuildContext, ButtonStyles, CornerRadii, EdgeInsets, Icon, Pressable, Row, SizeValue, SizeVariantValue, Spinner, StatelessComponent, StyleDiff, Text, TypeStyle, VariantValue } from "../runtime-exports";
...
export class Button extends StatelessComponent {
    ...
    constructor(label?: any, variant: any = 'primary', size: any = 'medium', onPressed: any = null, props?: any) {
        ...
        if (this.initialFocus === undefined) this.initialFocus = false;
    ...
    build(context: BuildContext) {
        ...
        let textColor = colors.onBase;
        let borderWidth = this.variant === 'outline' ? 1 : 0;
        let borderColor = theme.borderStrong;
        let inert = this.disabled || this.loading;
        if (inert) {
            let opacity = theme.disabledOpacity;
            let filled: any; 
            if ((filled = fill) != null) fill = filled.withOpacity(opacity);
            textColor = textColor.withOpacity(opacity);
            borderColor = borderColor.withOpacity(opacity);
        }
        let label = new Text(this.label, 'label', textColor, 1, 'start', false, false, null, 0, { styleOverride: TypeStyle.ofSize(labelSize, 'semiBold', Math.fround(0.1)) });
