import { Box, BoxStyle, BuildContext, Column, CornerRadii, EdgeInsets, Flexible, Icon, Row, SharedStatefulComponent, SizeValue, SizeVariantValue, Sizing, Text, TextEntry, UiComponent } from "../runtime-exports";
...
export class TextInput extends SharedStatefulComponent {
    ...
    build(context: BuildContext) {
        let theme = context.theme;let height = Sizing.height(this.size, context.density);let hasError = !!this.error;let borderColor = hasError ? theme.colors('destructive').base : this._focused ? theme.colors('primary').base : theme.borderStrong;let borderWidth = this._focused ? 2 : 1;let paddingX = this._focused ? 13 : 14;let caption = hasError ? this.error! : this.helper ?? '';let row = new Row(10, 'start', 'center', false, null, null, { height: SizeValue.fill, cross: 'center' });let leading: any; if ((leading = this.leading) != null) {row.add(new Icon(leading, 20, theme.textMuted));}row.add(new Flexible(new TextEntry(this.value, this.onChanged, { placeholder: this.placeholder, label: this.label.length > 0 ? this.label : null, description: caption.length > 0 ? caption : null, invalid: hasError, disabled: this.disabled, obscure: this.obscure, autofocus: this.autofocus, onFocusChanged: (focused: boolean) => {this.setState(() => this._focused = focused);this.onFocusChanged?.(focused);} }), 1));let container = new Box(new BoxStyle({ width: SizeValue.fill, height: height, background: this.disabled ? theme.surfaceSubtle : theme.surface, cornerRadius: new CornerRadii(theme.shape('medium')), borderWidth: borderWidth, borderColor: borderColor, padding: EdgeInsets.symmetric(paddingX, 0) }), row);let top = new Column(6, 'start', 'stretch', false, null, null, { width: SizeValue.fill });if (this.label.length > 0) top.add(new Text(this.label, 'label', theme.textSecondary));top.add(container);let captionColor = hasError ? theme.colors('destructive').base : theme.textMuted;let column = new Column(5, 'start', 'stretch', false, null, null, { width: SizeValue.fill });column.add(top);column.add(new Text(caption, 'caption', captionColor, 1));return column;
    }
