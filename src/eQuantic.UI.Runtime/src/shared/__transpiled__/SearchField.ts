...
export class SearchField extends StatelessComponent {
    ...
    build(context: BuildContext) {
        let theme = context.theme;let row = new Row(10, { cross: 'center' });row.add(new Icon('search', IconSize.md, theme.textMuted));row.add(new Flexible(new TextEntry(this.query, this.onChanged, { placeholder: this.placeholder, onSubmit: this.onSubmit, role: 'bodyM' }), 1));if (this.query.length > 0) {row.add(new Pressable(new Icon('close', IconSize.md, theme.textMuted), () => this.onChanged?.(''), { label: 'clear search' }));}return new Box(new BoxStyle({ width: SizeValue.fill, height: 40, background: theme.surfaceSubtle, cornerRadius: new CornerRadii(Radius.full), padding: EdgeInsets.symmetric(14, 0) }), row);
    }
