import { Anchored, Box, BoxStyle, BuildContext, Column, CornerRadii, EdgeInsets, Icon, IconSize, Pressable, Row, SharedStatefulComponent, SizeValue, Space, StyleDiff, Text, UiComponent, VisualNode } from "@equantic/runtime";
...
export class Menu extends SharedStatefulComponent {
    ...
    build(context: BuildContext) {
        let theme = context.theme;let list = new Column(0, { width: SizeValue.fill });for (let i = 0; i < this.items.length; i++) {let item = this.items[i];let index = i;let row = new Row(Space.s2, { cross: 'center', width: SizeValue.fill, height: SizeValue.fill });let glyph; if (((item.icon != null) && (glyph = item.icon, true))) row.add(new Icon(glyph, IconSize.dense, item.destructive ? theme.colors('destructive').base : theme.textSecondary));row.add(new Text(item.label, 'bodyM', item.destructive ? theme.colors('destructive').base : theme.textPrimary));let surface = new Box(new BoxStyle({ height: 40, padding: EdgeInsets.symmetric(Space.s3, 0), width: SizeValue.fill, opacity: item.disabled ? theme.disabledOpacity : null, hover: item.disabled ? null : new StyleDiff({ background: theme.surfaceSubtle }) }), row);list.add(item.disabled ? surface : new Pressable(surface, () => {this.onSelect?.(index);this.setState(() => this._open = false);}));}let panel = new Box(new BoxStyle({ minWidth: 180, background: theme.surface, cornerRadius: new CornerRadii(theme.shape('medium')), borderWidth: 1, borderColor: theme.border, elevation: 2, padding: EdgeInsets.symmetric(0, Space.s1), clip: true }), list);return new Anchored(new Pressable(this.trigger, () => this.setState(() => this._open = !this._open)), panel, { placement: this.placement, open: this._open, onDismiss: () => this.setState(() => this._open = false) });
    }
