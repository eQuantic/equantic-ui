...
export class Table extends StatelessComponent {
    ...
    build(context: BuildContext) {
        ...
        let tracks = GridTrack.repeat(this.columns.length, GridTrack.flex());
        ...
        for (let r = 0; r < this.rows.length; r++) {
            ...
            for (let c = 0; c < this.columns.length; c++) row.add(new Text(c < cells.length ? cells[c] : '', 'bodyM', null, 1));
            ...
            if (r < this.rows.length - 1) table.add(new Divider());
        }
        return table;
    }
}
