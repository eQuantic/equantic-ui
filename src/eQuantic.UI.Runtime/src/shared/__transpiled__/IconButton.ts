...
export class IconButton extends StatelessComponent {
    ...
    build(context: BuildContext) {
        let theme = context.theme;let primary = theme.colors('primary');let side = Sizing.height(this.size, context.density);let iconSize = (() => { const _s = this.size; if (_s === 'small') return 16; if (_s === 'medium') return 20; return 24; })();let fill = (() => { const _s = this.kind; if (_s === 'tonal') return primary.subtle; if (_s === 'filled') return primary.base; return null; })();let tint = (() => { const _s = this.kind; if (_s === 'filled') return primary.onBase; if (_s === 'tonal') return primary.onSubtle; return this.selected ? primary.base : theme.textSecondary; })();if (this.disabled) {let opacity = theme.disabledOpacity;let filled; if (((fill != null) && (filled = fill, true))) fill = filled.withOpacity(opacity);tint = tint.withOpacity(opacity);}let filledGlyph: any; let glyph = this.selected && ((this.selectedGlyph != null) && (filledGlyph = this.selectedGlyph, true)) ? filledGlyph : this.glyph;let content = new Icon(glyph, iconSize, tint).centered();let box = new Box(new BoxStyle({ width: side, height: side, background: fill, cornerRadius: new CornerRadii(theme.shape('full')), borderWidth: this.kind === 'outline' ? 1 : 0, borderColor: theme.borderStrong }), content);let pressedFill = (() => { const _s = this.kind; if (_s === 'filled') return primary.pressed; if (_s === 'tonal') return primary.pressed.withOpacity(0.24); return theme.surfaceSubtle; })();return new Pressable(box, this.disabled ? null : this.onPressed, { disabled: this.disabled, label: this.label, pressedBackground: this.disabled ? null : pressedFill });
    }
