import { $eq, AccordionItem, Box, BoxStyle, BuildContext, Column, Divider, EdgeInsets, Flexible, Icon, Pressable, Row, SharedStatefulComponent, SizeValue, Sizing, Spacer, StyleDiff, Text, UiComponent } from "@equantic/runtime";
...
export class Accordion extends SharedStatefulComponent {
    ...
    build(context: BuildContext) {
        let theme = context.theme;let column = new Column(0, 'start', 'stretch', false, null, null, { width: SizeValue.fill });for (let i = 0; i < this.items.length; i++) {let item = this.items[i];let index = i;let open = this.isOpen(i);let header = new Row(8, 'start', 'center', false, null, null, { cross: 'center', width: SizeValue.fill, height: SizeValue.fill });header.add(new Text(item.title, 'label'));header.add(new Flexible(new Spacer()));header.add(new Icon(open ? 'chevronUp' : 'chevronDown', 16, theme.textSecondary));column.add(new Pressable(new Box(new BoxStyle({ height: Sizing.height('large'), width: SizeValue.fill, padding: EdgeInsets.symmetric(12, 0), hover: new StyleDiff({ background: theme.surfaceSubtle }) }), header), () => this.toggle(index), { expanded: open }));let content: any; if (open && (content = item.content) != null) {column.add(new Box(new BoxStyle({ width: SizeValue.fill, padding: new EdgeInsets(12, 0, 12, 12) }), content));}if (i < this.items.length - 1) column.add(new Divider());}return column;
    }
