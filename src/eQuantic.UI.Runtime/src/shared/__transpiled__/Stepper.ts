import { Box, BoxStyle, BuildContext, CornerRadii, Icon, Pressable, Row, SizeValue, Sizing, StatelessComponent, Text } from "@equantic/runtime";
...
export class Stepper extends StatelessComponent {
    ...
    build(context: BuildContext) {
        let theme = context.theme;let height = Sizing.height(this.size, context.density);let canDecrement = !this.disabled && this.value - this.step >= this.min;let canIncrement = !this.disabled && this.value + this.step <= this.max;let row = new Row(0, { height: SizeValue.fill, cross: 'center' });row.add(Stepper.arm(theme, 'minus', height, canDecrement, () => this.onChanged?.(this.value - this.step), `Decrease ${this.label}`));let reading = new Row(0, { height: SizeValue.fill, main: 'center', cross: 'center' });reading.add(new Text(`${this.value}${this.suffix}`, 'label', this.disabled ? theme.textMuted : theme.textPrimary, 1, { align: 'center', tabular: true, styleOverride: theme.type('label').withSize(Sizing.labelSize(this.size, context.density)) }));row.add(new Box(new BoxStyle({ minWidth: height, height: SizeValue.fill }), reading));row.add(Stepper.arm(theme, 'plus', height, canIncrement, () => this.onChanged?.(this.value + this.step), `Increase ${this.label}`));return new Box(new BoxStyle({ width: SizeValue.hug, height: height, background: this.disabled ? theme.surfaceSubtle : theme.surface, cornerRadius: new CornerRadii(Sizing.radius(this.size)), borderWidth: 1, borderColor: theme.borderStrong, opacity: this.disabled ? theme.disabledOpacity : 1 }), row);
    }
