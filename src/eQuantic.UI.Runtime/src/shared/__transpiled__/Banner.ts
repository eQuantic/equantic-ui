import { Box, BoxStyle, BuildContext, Column, CornerRadii, EdgeInsets, Flexible, Icon, IconsValue, Pressable, Row, SdkStrings, SizeValue, StatelessComponent, Text, TypeStyle, VariantValue } from "@equantic/runtime";
...
export class Banner extends StatelessComponent {
    ...
    build(context: BuildContext) {
        ...
        column.add(new Text(this.title, 'caption', tint.onSubtle, 2, 'start', false, false, null, { styleOverride: new TypeStyle(13, 18, 'semiBold', 0, 1.3) }));
        if (this.body != null) {
            column.add(new Text(this.body, 'caption', tint.onSubtle, 4, 'start', false, false, null, { styleOverride: new TypeStyle(13, 18, 'regular', 0, 1.3) }));
        }
        if (this.primaryAction != null || this.secondaryAction != null) {
            let actions = new Row(8);
            if (this.primaryAction != null) actions.add(this.primaryAction);
            if (this.secondaryAction != null) actions.add(this.secondaryAction);
            column.add(actions);
        }
