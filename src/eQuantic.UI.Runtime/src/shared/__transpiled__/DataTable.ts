import { $eq, Box, BoxStyle, BuildContext, Checkbox, Column, DataColumn, DataRow, EdgeInsets, Grid, GridTrack, Icon, Motion, Pressable, Row, SizeValue, Skeleton, StatelessComponent, StyleDiff, Text, TextAlignmentValue, TransitionSpec, VisualNode } from "@equantic/runtime";
...
export class DataTable extends StatelessComponent {
    ...
    build(context: BuildContext) {
        ...
        for (const row of this.rows) table.add(this.body(theme, tracks, row));
        for (let i = 0; i < this.pendingRows; i++) table.add(this.pending(theme, tracks, i));
        return table;
    }
