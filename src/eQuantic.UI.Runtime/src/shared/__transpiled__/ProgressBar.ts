import { $eq, Box, BoxStyle, BuildContext, CornerRadii, Flexible, LoopMotion, Row, SharedStatefulComponent, SizeValue, Spacer, UiComponent, VariantValue } from "@equantic/runtime";
...
export class ProgressBar extends SharedStatefulComponent {
    ...
    adoptConfig(next: UiComponent) {
        let fresh: any; 
        if (!((next instanceof ProgressBar && (fresh = next, true)))) return;
        let incoming: any; let current: any; 
