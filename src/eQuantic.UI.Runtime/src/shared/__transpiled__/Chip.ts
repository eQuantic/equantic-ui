import { Box, BoxStyle, BuildContext, CornerRadii, EdgeInsets, Icon, Pressable, Row, SdkStrings, SizeValue, Sizing, StatelessComponent, StyleDiff, Text, TypeStyle, VariantValue } from "@equantic/runtime";
...
export class Chip extends StatelessComponent {
    ...
    build(context: BuildContext) {
        let theme = context.theme;let primary = theme.colors('primary');let tint = theme.colors(this.variant);let fill = (() => { const _s = this.kind; if ((_s === 'filter') && (this.selected)) return primary.subtle; if (_s === 'filter') return theme.surfaceSubtle; if (_s === 'tag') return tint.subtle; return theme.surfaceSubtle; })();let textColor = (() => { const _s = this.kind; if ((_s === 'filter') && (this.selected)) return primary.onSubtle; if (_s === 'tag') return tint.onSubtle; return theme.textSecondary; })();let label = new Text(this.label, 'caption', textColor, 1, 'start', false, false, null, { styleOverride: new TypeStyle(13, 16, 'semiBold', 0, 1.3) });let content = new Row(6, 'start', 'center', false, null, null, { main: 'center', height: SizeValue.fill });if (this.kind === 'filter' && this.selected) {content.add(new Icon('check', 16, textColor));}content.add(label);if (this.kind === 'input' && this.onRemove != null) {content.add(new Pressable(new Icon('close', 20, textColor), this.onRemove, { label: SdkStrings.remove }));}let hoverFill = this.kind === 'filter' && this.onPressed != null ? this.selected ? primary.subtle.midpointWith(primary.base) : theme.surfaceSubtle.midpointWith(theme.border) : null;let box = new Box(new BoxStyle({ height: Sizing.height('small', context.density), padding: EdgeInsets.symmetric(12, 0), background: fill, cornerRadius: new CornerRadii(theme.shape('full')), borderWidth: this.kind === 'filter' && this.selected ? 1 : 0, borderColor: primary.base, hover: hoverFill == null ? null : new StyleDiff({ background: hoverFill }) }), content);return this.kind === 'filter' && this.onPressed != null ? new Pressable(box, this.onPressed, { label: this.label, pressedBackground: this.selected ? primary.pressed.withOpacity(0.24) : theme.surfaceSubtle, selected: this.selected }) : box;
    }
