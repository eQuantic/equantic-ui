import { AdaptiveNode, Adjustable, Anchored, Avatar, Badge, Banner, BottomNavigation, Box, BoxStyle, Button, Card, Checkbox, Chip, ColorToken, Column, CultureOption, CultureSwitcher, Dialog, DialogAction, Divider, DragDismiss, Draggable, Drawer, Drawing, EdgeInsets, EmptyState, Flexible, Grid, GridTrack, Hoverable, Icon, IconButton, IconGlyph, Image, InFlow, InView, KeyChord, Link, Markdown, Mermaid, NavigationRail, NavItem, Overlay, Positioned, Presence, Pressable, ProgressBar, Row, SafeArea, ScrollView, SearchField, Select, Shortcut, Simulated, Skeleton, Slider, Spacer, Spinner, Stack, Stepper, Sticky, Switch, Tabs, Text, TextEntry, TextInput, Toast, Tooltip, TypeStyle, Vector, VectorDrawing, VisualNode } from "@equantic/runtime";
export class UI {
    ...
    static row(gap: number = 0, main: string = 'start', cross: string = 'center', wrap: boolean = false, runGap: number | null = null, padding: EdgeInsets | null = null, children: VisualNode[] | null = null) { let node = new Row(gap, main, cross, wrap, runGap, padding);if (children != null) for (const child of children) node.add(child);return node; }
    ...
    static text(content: string, role: string = 'bodyL', color: ColorToken | null = null, maxLines: number = 0, align: string = 'start', mono: boolean = false, tabular: boolean = false, styleOverride: TypeStyle | null = null) { return new Text(content, role, color, maxLines, align, mono, tabular, styleOverride); }
