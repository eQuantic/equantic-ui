...
export class SharedCounter extends SharedStatefulComponent {
    ...
    build(_context: BuildContext) {
        let column = new Column(Space.s3);column.add(new Text(`Count: ${this._count}`, 'title'));for (const cell of this.cells()) column.add(cell);column.add(new Button('Increment', 'primary', 'medium', () => this.setState(() => this._count++)));column.add(new Button('Later', 'primary', 'medium', this.bumpSoon.bind(this)));return column;
    }
