...
{
    /// <summary>
    /// A CURATED glyph is a glyph, so <c>Icon(Icons.Search)</c> needs no second constructor to say
    /// so. <see cref="Icon"/> had one per family and they differed only in their first parameter,
    /// which is the shape the repo's no-overloads rule exists to remove — and the TypeScript twin
