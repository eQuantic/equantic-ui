...
| 2b, the engine | delivered by [#386](https://github.com/eQuantic/equantic-ui/pull/386): `CodeDiffer` answers what differs between two texts, by lines (Myers' shortest edit script, the one `git diff --minimal` computes, over what is left once the common head and tail are trimmed) and, inside each change, by words (runs of word characters and of whitespace, any other character alone, a surrogate pair whole, the break between two lines); past 2,000 rounds two ranges are a rewrite, marked whole, at the same round on .NET and on the web. Pinned by hand, against a longest common subsequence on 500 random pairs, against `git diff --minimal --numstat` on five files of this history, and against its twin change by change. Found on the way: eqc named a method by its name alone, so two overloads reached a twin as one and every emitter lost one of them in silence; the build now refuses the second (EQ1007); and white space is .NET's on the web, which the twin's tokenizer was not (U+0085 in, U+FEFF out), with the `Trim` family, `IsNullOrWhiteSpace` and a bare `Split()` |
| 2b, the view | delivered by [#412](https://github.com/eQuantic/equantic-ui/pull/412): `CodeDiff`, side by side or inline, over two texts or one file of a patch (§9): the sides level at every change, a changed line washed and its words marked, unchanged runs folded into a row that opens on a press, F7, Shift+F7 and the toolbar stepping through the changes both ways, each side numbered as its file, and the modified side editing and compared again after every edit. Under it the engine maps a view's lines to rows (`CodeRows`, `CodeFiller`, `CodeCollapse`), `CodeDiffLayout` lays out each side, `CodePatch` reads a unified diff, and `CodeBlock` draws rows. Found on the way: four shapes eqc crossed wrong (an array of a union, a tuple return, a local starting null, a function-typed parameter; a negative declared default was #409's to settle), an extended property pattern that read `changes.Count`, a button inside a code surface that never heard its click on the web, a page-wide `Shortcut` answering for the last diff mounted (`Shortcut.FocusScoped` makes a chord the subtree's own, and the editor's ⌘F uses it), and a Mac's function keys reaching the host as the characters they type. The author's review found six more, each fixed (a local annotated in plain JavaScript, a zero-context patch's gaps, F7 past a removed end, the focus a fold's press dropped on the web, a patch line's bare carriage return, a tuple's enum), and the find bar's page-wide Escape is [#457](https://github.com/eQuantic/equantic-ui/issues/457) |
