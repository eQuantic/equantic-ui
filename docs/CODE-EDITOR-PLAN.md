...
| 2b, the view | in review as [#412](https://github.com/eQuantic/equantic-ui/pull/412): `CodeDiff`, side by side or inline, over two texts or one file of a patch (§9): the sides level at every change, a changed line washed and its words marked, unchanged runs folded into a row that opens on a press, F7, Shift+F7 and the toolbar stepping through the changes both ways, each side numbered as its file, and the modified side editing and compared again after every edit. Under it the engine maps a view's lines to rows (`CodeRows`, `CodeFiller`, `CodeCollapse`), `CodeDiffLayout` lays out each side, `CodePatch` reads a unified diff, and `CodeBlock` draws rows. Found on the way: five shapes eqc crossed wrong (an array of a union, a tuple return, a local starting null, a function-typed parameter, a negative declared default), an extended property pattern that read `changes.Count`, a button inside a code surface that never heard its click on the web, a page-wide `Shortcut` answering for the last diff mounted (`Shortcut.FocusScoped` makes a chord the subtree's own, and the editor's ⌘F uses it), and a Mac's function keys reaching the host as the characters they type |
