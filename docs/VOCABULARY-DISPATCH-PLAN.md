...
realizer; the dispatch had twelve it never handled. `UiComponent` was invisible to it by
construction — the pin asked its question of non-abstract nodes, and that one is abstract — so the
instrument that existed to stop an omission passing for a decision could not see this one at all.
The compiler demanded the door on the first build.
...
| S7 | **Landed ([#179](https://github.com/eQuantic/equantic-ui/issues/179)).** `NodeKindTsGenerator` writes `node-kinds.generated.ts` — 39 declared kinds in ordinal order plus the seam, which is added explicitly because `UiComponent` is abstract and no scan of concrete types reaches it. `VisualNodeValue.nodeKind` and the runtime `VisualNode`'s own are that union now instead of `string`, and `lowerNodeKind` ends in `assertNever`. PROVEN, not assumed: deleting the `spinner` arm makes `tsc` say *Argument of type `'spinner'` is not assignable to parameter of type `never`* — the browser's half of one door per node. The mixing seam moved AHEAD of the switch, and its test changed with it: a web component carries no `nodeKind` at all (`HtmlElement` is an `IComponent`, never a `VisualNode`), so the seam asks for the discriminator's ABSENCE, which is the truth about those objects, where the old default arm asked for a case the union no longer permits. `EveryNode_DeclaresItsOwnWireKind`'s three rules are the generator's refusals now — empty kind, duplicate kind, a concrete node claiming the seam's word — so S8 deletes a file and loses no check. The TypeScript dispatch left the pin, and took `Language`, `WireKind` and the whole TypeScript branch of the matcher with it; three dispatches remain, all C#. **THE DECISION, taken rather than deferred: NO.** A page does not walk a tree. `Accept` stays `[ServerOnly]`, the 39 twins get no `accept`, and there is no TypeScript `IVisualNodeVisitor` — nothing in the tree or the samples asks to visit from a page, and the cost is 39 methods and an interface that would exist for no caller. It is revisited when a page has a reason, and this row is where that reason gets argued. | `NodeKindTsGeneratorTests` beside `EnumUnionsTsGeneratorTests` (byte-pinned, `EQ_UPDATE_NODE_KINDS_TS=1`); `tsc` over the runtime, which is what makes `assertNever` bite, then 128 files / 1,058 vitest tests unchanged | S |
...
4. **Does a page ever walk a tree?** *(settled at S7: no.)* Saying yes costs `accept` on all 39 twin
   classes and a TypeScript `IVisualNodeVisitor`, and buys a capability nothing asks for — `Accept`
   has been `[ServerOnly]` since S1 and no page, sample or consumer has wanted to visit. It is taken
   the day one does, and not by default.
