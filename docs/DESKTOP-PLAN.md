...
| W-B4 | Hit-testing is rectangle-only | `PhotonHost.cs:1532-1560` — every pointer resolution is `Bounds.Contains(point)` on AABBs; `OnPressed` carries no coordinates |
...
- **The 120 Hz driver exists but is a run-loop timer**, not a display link
  (`PhotonWindow.cs:298` — `CFRunLoopTimer` at 1/120). W2 should swap it for `CVDisplayLink` (or
