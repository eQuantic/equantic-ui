...
| `SingleChildRenderObjectWidget`, `MultiChildRenderObjectWidget`, `LeafRenderObjectWidget` — the SHAPES a node can take, named once | `SingleChildNode` names the first; `FlexNode` covers the flex containers only; the leaves share no base | **PARTIAL — one shape of three.** `SingleChildNode` holds the twenty nodes whose child is REQUIRED, so "a layout-transparent wrapper" is a type the compiler knows rather than a list each realizer keeps. `Box` is deliberately not one: its child is optional, and a base meaning "wraps a child" would have to make `Child` nullable for all twenty to admit the one. The other two shapes are still unnamed — `FlexNode`, `Grid`, `Stack` and `Navigable` each hold many through a collection of their own, and the eleven leaves are leaves only by having no child property. |
