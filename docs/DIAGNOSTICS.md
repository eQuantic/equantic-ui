...
| `EQ4003` | *(warning)* The app will be signed with a hardened-runtime exception (`com.apple.security.cs.*` — the family that relaxes a protection the hardened runtime imposes) while `EQuanticHardenedRuntime` is explicitly `false`. The key reaches the signature and the system never consults it. | Remove `EQuanticHardenedRuntime=false` — setting `EQuanticSigningIdentity` turns hardening on by itself — or drop the declaration. Note what this does NOT mean: the App Sandbox's permissions (`com.apple.security.network.*`, `…files.*`, `…device.*`) answer to `PhotonEntitlements.AppSandbox`, not to the hardened runtime, and an ad-hoc development build honours them — so they are never reported here. |
