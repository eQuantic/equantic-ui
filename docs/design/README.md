...
| `Photon-Design-System.dc.html` | The EARLIER single-file export (378 KB), cited by `PhotonTheme` and `DesignTokenTests` as §01–§06. Kept until those citations are moved: it is larger than the new split set and nothing has yet proved the split carries all of it. |
