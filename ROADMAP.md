...
> Last updated: 2026-06-09. This document is an honest, evidence-based assessment of where
...
hydration mis-aligning on whitespace). These have been fixed, but their *nature* is the real signal:
the correctness net is not yet in place.

### Evidence snapshot

| Area | State |
|------|-------|
| Transpiler | 118 strategies; **~69 "not supported / TODO / fallback" markers**; correctness was discovered-by-failure, not specified |
...
### 1. Zero JavaScript — **the linchpin, and the biggest gap**
The "0 JS" promise is worth exactly as much as the transpiler is **complete and correct**. The bugs
fixed were silent miscompilations — the worst failure mode for a C#-only developer, because the
symptom appears in the browser with no C# vocabulary to debug it. Missing:
- A **defined, documented supported subset** of C# (today support is implicit).
- **Compile-time errors for unsupported constructs** — never silent wrong output.
- A **conformance suite that executes the emitted JS (via Bun) and compares to .NET** behavior.
- **Reliable source maps + C# stack traces** in the browser.
...
- **Phase 1 — Transpiler correctness & conformance** *(linchpin)*: supported-subset spec,
  fail-on-unsupported diagnostics, conformance harness (run emitted JS via Bun, compare to .NET),
  source-map validation. → see `docs/IMPLEMENTATION-PLAN.md`.
