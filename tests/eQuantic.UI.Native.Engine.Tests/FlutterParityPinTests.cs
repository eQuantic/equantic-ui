...
public class FlutterParityPinTests
{
    ...
    private static readonly Dictionary<string, Func<bool>> Probes = new()
    {
        ...
        // The shapes, ONE of three named: SingleChildNode holds the twenty-two wrappers whose child
        ...
        // that wraps one required child IS one (the rule), and there are twenty-two of them (the
        // measurement this PR's prose and the audit both quote; twenty-three nodes wrap one child and
        ...
        ["SingleChildRenderObjectWidget"] = () => EveryOneChildNodeIsA("SingleChildNode")
            && Descendants("SingleChildNode") == 22 && Has("FlexNode")
