...
public class VectorPathParserTests
{
    [Fact]
    public void AbsoluteMoveLine_Close_ParsesToVerbs()
    {
        var segments = VectorPath.Parse("M4 3h5v7L4 10z");

        segments.Should().HaveCount(5);
        segments[0].Verb.Should().Be(VectorVerb.Move);
        segments[1].Verb.Should().Be(VectorVerb.Line);
        segments[1].End.X.Should().Be(9);   // h5 is relative-horizontal
        segments[1].End.Y.Should().Be(3);
        segments[2].End.Y.Should().Be(10);  // v7
        segments[4].Verb.Should().Be(VectorVerb.Close);
    }

    [Fact]
    public void CompactNumbers_AndNegatives_Lex()
    {
        var segments = VectorPath.Parse("M1.5.5l-1-1");
    ...
    {
        var segments = VectorPath.Parse("M0 0Q3 0 3 3");

        segments[1].Verb.Should().Be(VectorVerb.Cubic);
    ...
    public void Arc_ConvertsToCubics_EndingExactly()
    {
        ...
        var segments = VectorPath.Parse("M0 0a5 5 0 0 1 10 0");
    ...
    public void CuratedGlyphs_AllParse()
    {
        foreach (Primitives.Icons glyph in Enum.GetValues<Primitives.Icons>())
        {
            ...
            VectorPath.Parse(resolved.Path).Should().NotBeEmpty(
    ...
    {
        var segments = VectorPath.Parse("M0 0 5 5 Xnope");
